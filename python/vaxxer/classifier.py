...
class VaxxerClassifier:
    ...
    def _set_experiment_info(self, feature_dir: str):
        if not os.path.exists(feature_dir) or len(os.listdir(feature_dir)) == 0:
            raise RuntimeError(
                "You must generate feature components before model training!"
            )
        else:
            if "/" == feature_dir[-1]:
    ...
    def _load_labels_and_meta(self, folders: list):
        if "Labels" in folders:
            ...
            if os.path.exists(test_label_fp):
                ...
                print(
                    "Train-test label size:", self.tr_label.shape, self.te_label.shape
                )
            else:
                self.te_label = None
            ...
            raise RuntimeError(
                "'Labels' folder must be present to load labels for the classification task!"
            )
    ...
    def set_components(self, fixed: list, excluded: list):
        ...
        for component_path in fixed:
            ...
            if len(splitted) == 2 and splitted[0] in component_types:
                ...
                raise RuntimeError(
                    "'%s' component type is both present in the excluded and fixed components. Remove it from the excluded components!"
                    % splitted[0]
                )
            elif len(splitted) > 2:
                raise ValueError(
                    "Too long component path were set: %s. Provide only a relative path from the root of the feature directory!"
                    % component_path
                )
                ...
                if len(subfolders) > 0:
                    for subfolder in subfolders:
                        ...
                        if comp_type == "Bert":
                            components[comp_type][
                                subfolder
                            ] = self._load_bert_component(comp_dir)
                        else:
                            components[comp_type][
                                subfolder
                            ] = self._load_general_component(comp_dir)
                        pbar.update(1)
                else:
                    raise RuntimeError(
                        "%s component type should have multiple options!" % comp_type
                    )
        if "Bert" in components or "TfIdf" in components:
            self.components = components
        else:
            raise RuntimeError(
                "Tweet text representation in missing! Generate text representation with 'TfIdfComponent' or 'BertComponent'!"
            )
    ...
    def _generate_embeddings(self, config: dict):
        ...
        if self._embedding_type == "bert":
            train_ids, train_mask, test_ids, test_mask, cols = self.components["Bert"][
                config["Bert"]
            ]
            ...
            if len(tr_parts) > 0:
                ...
                if sample_seed != None:
                    train_ids, train_mask, z_tr, y_tr = resample(
                        train_ids,
                        train_mask,
                        z_tr,
                        y_tr,
                    ...
                    test_ids, test_mask, z_te, y_te = resample(
                        test_ids,
                        test_mask,
                        z_te,
                        y_te,
                ...
                if sample_seed != None:
                    train_ids, train_mask, y_tr = resample(
                        train_ids,
                        train_mask,
                        y_tr,
                        replace=True,
                    ...
                    test_ids, test_mask, y_te = resample(
                        test_ids,
                        test_mask,
                        y_te,
                        replace=True,
            ...
            if sample_seed != None:
                X_train, self.tr_label = resample(
                    X_train,
                    self.tr_label,
                    replace=True,
                    random_state=sample_seed,
    ...
    def _run_single_config(
        self, config: dict, return_probas=False, show_conf_mx=False, verbose=False
    ):
        ...
        if (
            self._embedding_type != "bert" and self.model is None
        ):  # shallow classification
            solver = config.get("solver", "newton-cg")
            clf = LogisticRegression(solver=solver)
            metrics_df, train_probas, test_probas = fit_and_evaluate_sklearn_model(
                clf,
                X_train,
                X_test,
                self.tr_label,
            ...
            batch_size, epochs, lr_rate, schedule_type = (
                config.get("batches", 32),
                config.get("epochs", 20),
                config.get("lr_rate", 0.001),
            ...
            num_labels = (
                self.tr_label.shape[1] if "Multiclass" == self.class_label else 2
            )
            if self._embedding_type != "bert":  # NeuralModel classification
                ...
                if self.model == "lstm":
                    ...
                    clf = LSTMClassifier(
                        ...
                        dropout,
                        use_lstm=True,
                        lstm_dim=2 * inter_dim,
                        maintain_lstm_state=maintain_lstm,
                        device=self.device,
                        show_confusion_matrix=show_conf_mx,
                        verbose=verbose,
                    )
                else:
                    clf = TextClassifier(
            ...
            metrics_df, train_probas, test_probas = clf.fit_eval_predict(
                X_train, X_test, return_probas
            )
    ...
    def tune(
        ...
        def objective(trial):
            ...
            tmp_df = (
                tmp_df.groupby(group_cols)["score"].agg(["mean", "std"]).reset_index()
            )
    ...
    def export_probas(self, export_dir, parameters, best_config):
        ...
        best_metrics, train_probas, test_probas = self._run_single_config(
            augmented_best_config, return_probas=True
        )
        print(
            best_metrics[["part", "metric", "score"]].sort_values(
                ["part", "metric"], ascending=True
            )
        )
