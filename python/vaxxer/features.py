...
from .utils import LabelIndexer, get_train_test_data, user_label_encoding, get_twitter_stats, get_centralities, load_node_embeddings
...
class Feature():
    ...
    def generate(self, configuration):
        ...
        if self.preprocessed:
            ...
            raise RuntimeError("The preprocess() function was not executed! Please execute it before generating feature components!")
    
class TextFeature(Feature):
    """Abstract object for a general text based feature component"""
    def __init__(self, tr_meta, te_meta, tr_text, te_text, stem=False, lemmatize=False, normalize=False):
...
class TfIdfComponent(TextFeature):
    ...
    def __init__(self, tr_meta, te_meta, tr_text, te_text, stem=False, lemmatize=False, only_emoji=False, normalize=False):
        super(TfIdfComponent, self).__init__(tr_meta, te_meta, tr_text, te_text, stem, lemmatize, normalize)
        self.only_emoji = only_emoji
        
    def _preprocess_sentences(self, sentences):
        return preprocess_sentences(sentences, self.stem, self.lemmatize, only_emoji=self.only_emoji, lowercase=True, stopwords=True, tokenized_output=False)
...
class BertComponent(TextFeature):
    ...
    def __init__(self, tr_meta, te_meta, tr_text, te_text, bert_model='ans/vaccinating-covid-tweets', max_tensor_len=120, stem=False, lemmatize=False, normalize=False):
        if normalize:
            raise ValueError("BertComponent cannot be normalized!")
        super(BertComponent, self).__init__(tr_meta, te_meta, tr_text, te_text, stem, lemmatize, normalize)
        self.bert_model = bert_model
        self.max_tensor_len = max_tensor_len
        
    def _preprocess_sentences(self, sentences):
        return preprocessing_bert(sentences, self.bert_model, self.max_tensor_len, self.stem, self.lemmatize)
    ...
    def _generate(self, config):
        ...
        return (input_ids, masks), (te_input_ids, te_masks), columns, dict2str(cfg, self.normalize, "Bert")

class UserTwitter(Feature):
    """Extract Twitter statistics for users"""
    def __init__(self, tr_meta, te_meta, columns=['usr_followers_count', 'usr_friends_count', 'usr_favourites_count', 'usr_statuses_count'], record_id="id_str", user_id="usr_id_str", normalize=False):
    ...
    def _preprocess(self):
        tr_preproc = get_twitter_stats(self.tr_meta, self.record_id, self.user_id, self.columns, logscale=True, use_int=False)
        te_preproc = get_twitter_stats(self.te_meta, self.record_id, self.user_id, self.columns, logscale=True, use_int=False)
        ...
        agg_df = tmp_df.groupby(self.user_id)[self.columns].agg([relative_change,"nunique"]).reset_index()
        tr_preproc = tr_preproc.merge(agg_df, on=self.user_id, how="left")
        te_preproc = te_preproc.merge(agg_df, on=self.user_id, how="left")
        self.columns = list(tr_preproc.drop([self.record_id, self.user_id], axis=1).columns)
    ...
    def _generate(self, config):
        if config.get("user_twitter", False):
            ...
            return train_df.values, test_df.values, self.columns, dict2str({}, self.normalize, "Twitter")
        else:
            return None, None, None, None
    
class UserHistory(Feature):
    """Aggregate historical labels for users"""
    def __init__(self, tr_meta, te_meta, tr_label, te_label, record_id="id_str", user_id="usr_id_str", normalize=False):
    ...
    def _generate(self, config):
        if config.get("user_history", False):
            ...
            return train_df[columns].values, test_df[columns].values, columns, dict2str({}, self.normalize, "History")
        else:
            return None, None, None, None
        
class UserCentrality(Feature):
    """Network centrality values for users"""
    def __init__(self, tr_meta, te_meta, centrality_file, record_id="id_str", user_id="usr_id_str", normalize=False):
    ...
    def _preprocess(self):
        self.tr_preproc = get_centralities(self.tr_meta, self.record_id, self.user_id, self.centrality_df)
        self.te_preproc = get_centralities(self.te_meta, self.record_id, self.user_id, self.centrality_df)
    
    def _generate(self, config):
        if config.get("user_centrality", False):
            train_df = self.tr_preproc[self.columns].fillna(0.0)
            test_df = self.te_preproc[self.columns].fillna(0.0)
            return train_df.values, test_df.values, self.columns, dict2str({}, self.normalize, "Centrality")
        else:
            return None, None, None, None
        
class NodeEmbedding(Feature):
    """Node embedding for users"""
    def __init__(self, tr_meta, te_meta, node_emb_dir, file_names=None, record_id="id_str", user_id="usr_id_str", normalize=False):
    ...
    def _generate(self, config):
        ...
        if fname in self.tr_preproc:
            ...
            cfg = self.config(config)
            return train_df.values, test_df.values, columns, dict2str(cfg, self.normalize, "Network")
        else:
            return None, None, None, None
