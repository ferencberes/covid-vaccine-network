...
class FeatureGenerator():
    def __init__(self, seed_fp: str, label_fp: str, output_dir: str, class_label: str="Multiclass", drop_irrelevant: bool=True, meta_cols: list=['id_str', 'usr_id_str', 'epoch', 'usr_followers_count', 'usr_friends_count', 'usr_favourites_count', 'usr_statuses_count'], train_ratio: float=0.7, verbose: bool=True, tweet_filter=None):
    ...
    def _load(self):
        start = time.time()
        self.tr_text, tr_label, self.tr_meta, self.te_text, te_label, self.te_meta, unlabeled_text, unlabeled_meta = get_train_test_data(self.seed_fp, self.label_fp, train_ratio=self.train_ratio, meta_cols=self.meta_cols, drop_irrelevant=self.drop_irrelevant, visualize=False, verbose=self.verbose, tweet_ids=self.tweet_ids)
    ...
    def __str__(self):
        return "%s_di%s_tr%.2f" % (self.class_label, self.drop_irrelevant, self.train_ratio)
    ...
    def _create_folders(self, component: Feature, setting_str: str):
        ...
        if len(splitted_component) > 1:
            ...
            export_dir = os.path.join(self.output_dir, str(self), component_type, component_params)
        else:
