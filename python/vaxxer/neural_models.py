...
class NeuralNet(nn.Module):
    ...
    def forward(self, data, labeled=False, states=None, return_dict=True):
        ...
        if self.use_lstm:
            if (
                len(x.size()) < 3
            ):  # lstm expects 3d array as input: (batch, seq_len, feature)
                x = x.unsqueeze(1)
            dim_match = 0  # at last step states.size() might not match with shape of x
            if states is not None and x.size(0) != states[0].size(1):
                dim_match = states[0].size(1) - x.size(
                    0
                )  # cat zeros to x to match size of states
                if dim_match > 0:
                    augmentation = torch.zeros(dim_match, x.size(1), x.size(2)).to(
                        x.device
                    )
                    x = torch.cat([x, augmentation], dim=0)
                else:
                    augmentation = torch.zeros(1, -dim_match, states[0].size(2)).to(
                        states[0].device
                    )
                    states = [
                        torch.cat([state, augmentation], dim=1) for state in states
                    ]
...
class NeuralModels:
    ...
    def _epoch(self, dataloader: DataLoader, train: bool = True, labeled: bool = False):
        ...
        self._save_model(
            result, model, optimizer, scheduler, probas, truth, train, labeled
        )
    ...
    def fit_eval_predict(
        self, train_data: TensorDataset, test_data: TensorDataset, return_probas=False
    ):
        ...
        train_metrics = calculate_metrics(
            self.truth, self.probas, self.show_confusion_matrix, self.verbose
        )
        ...
        self.close()
        return (
            metrics2df({"train": train_metrics, "test": test_metrics}),
            train_probas,
            test_probas,
        )
...
class LSTMClassifier(NeuralModels):
    def __init__(
        ...
        self.model = NeuralNet(
            ...
            use_lstm=use_lstm,
            lstm_dim=lstm_dim,
            maintain_lstm_state=maintain_lstm_state,
        )
        self.model.to(self.device)
