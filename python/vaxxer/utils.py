...
def temporal_train_test_split(
    labeled_df, train_ratio=0.6, time_col="epoch", verbose=True
):
    ...
    if verbose:
        ...
        print(
            labeled_seeds_df["label"].value_counts().sort_index()
            / len(labeled_seeds_df)
        )
...
def get_train_test_data(
    ...
    seed_df = pd.merge(
        seed_df,
        label_df.drop(["usr_id_str", "is_conv_seed"], axis=1),
        on="id_str",
        how="left",
    )
    ...
    train_df, test_df = temporal_train_test_split(
        labeled_df, train_ratio, verbose=verbose
    )
    ...
    tr_meta, te_meta, unlabeled_meta = (
        train_df[meta_cols],
        test_df[meta_cols],
        unlabeled_df[meta_cols],
    )
    return (
        tr_text,
        tr_label,
...
def fit_and_evaluate_sklearn_model(
    ...
    results = {}
    results["train"] = calculate_metrics(
        y_train, train_probas, show_confusion_matrix, verbose
    )
    if not y_test is None:
        results["test"] = calculate_metrics(
            y_test, test_probas, show_confusion_matrix, verbose
        )
    if return_probas:
...
def evaluate_classifiers(
    ...
    if vectors_test is None:
        train_test_data = train_test_split(
            vectors, labels, random_state=0, test_size=test_size
        )
    else:
        train_test_data = (vectors, vectors_test, labels, labels_test)
    results = {}
    for i in range(len(classifiers)):
        results[names[i]] = evaluate_classifier(
            classifiers[i], train_test_data, multiclass, show_confusion_matrix, verbose
        )
...
def dynamic_auc(
    df, window=7 * 86400, epoch_col="epoch", label_col="label", proba_col="proba"
):
    """Calculate AUC over time with the pre-defined time window."""
    tmp_df = df.copy()
    tmp_df["date"] = pd.to_datetime(tmp_df[epoch_col], unit="s").apply(
...
def extract_frequent_word_by_label(
    seed_fp,
    label_fp,
    tweet_ids=None,
    forbidden=[
    ...
    tweet_text, tweet_label, _, _, _, _, _, _ = get_train_test_data(
        seed_fp,
        label_fp,
        train_ratio=1.0,
        drop_irrelevant=False,
    ...
    skeptic_high_freq = filter_high_freq_words(
        skeptic_data_freq, all_data_freq, forbidden
    )
    provax_high_freq = filter_high_freq_words(
        provax_data_freq, all_data_freq, forbidden
    )
    irrelevant_high_freq = filter_high_freq_words(
        irrelevant_data_freq, all_data_freq, forbidden
    )
    return (
        skeptic_high_freq,
        provax_high_freq,
        irrelevant_high_freq,
        skeptic_data,
        provax_data,
        irrelevant_data,
        all_data_freq,
...
def show_node_embedding_performance(network_auc_df, width=800, height=400):
    ...
    text_auc = float(
        visu_df[(visu_df["Network"] == "None") & (visu_df["History"] == "None")]["mean"]
    )
    text_history_auc = float(
        visu_df[(visu_df["Network"] == "None") & (visu_df["History"] == "norm:True")][
            "mean"
        ]
    )
    print(text_auc, text_history_auc)
    visu_df = visu_df[visu_df["Network"] != "None"]
    visu_df["History"] = visu_df["History"].replace(
        {"None": "text + raw network", "norm:True": "text + user history + raw network"}
    )
    fig = px.bar(
        visu_df,
        x="Network",
...
def show_dynamic_auc(
    configs,
    predictions,
    badrate,
    window=7 * 86400,
    markers=["circle", "square", "star", "triangle-up"],
):
    ...
    fig.add_trace(
        go.Scatter(
            x=dates, y=badrate[dates], name="Vax-skeptic rate", line=dict(dash="dot")
        ),
        row=1,
        col=1,
        secondary_y=True,
...
def divide_pro_and_skeptic(visu_df, seeds_df, id_subset=None):
    ...
    skeptic_df, skeptic_tweets = filter_for_vaccine_view(
        visu_df, seeds_df, 1, id_subset
    )
...
def merge_user_information(pred_df, X, coords=None, show_hist=False):
    ...
    pred_tmp_df["label"] = pred_tmp_df["usr_id_str"].apply(
        lambda x: round(label_map[x])
    )
...
def clean_text(
    df, text_col="full_text", new_col="clean_text", stem=False, lemmatize=True
):
...
def prepare_data_for_topic_visualization(
    user_df, seed_df, tweet_ids, stem=False, lemmatize=True
):
    skeptic_df, skeptic_tweets, pro_df, pro_tweets = divide_pro_and_skeptic(
        user_df, seed_df, tweet_ids
    )
    print("Cleaning text STARTED...")
    skeptic_tweets, skeptic_word_counter = clean_text(
        skeptic_tweets, stem=stem, lemmatize=lemmatize
    )
    pro_tweets, pro_word_counter = clean_text(
        pro_tweets, stem=stem, lemmatize=lemmatize
    )
...
def extract_word_representations(
    ...
    word_df = (
        word_df.groupby(key_columns)[aggregated_columns]
        .agg(["count", "mean", "std"])
        .reset_index()
    )
    word_df.columns = new_columns
    word_df = word_df[word_df["x_cnt"] > min_count].sort_values(
        "x_cnt", ascending=False
    )
...
def calculate_sequential_stats(
    df, id_col, user_col, value_cols=None, aggregations=["mean"], historical=True
):
    ...
    for agg_type in aggregations:
        ...
        tmp_df = pd.DataFrame(
            agg_records[agg_type_str],
            index=agg_df.index,
            columns=[agg_type_str + "_" + col for col in value_cols],
        )
...
def append_meta_cols(X, meta_df, cols):
    df_with_ids = pd.concat(
        [meta_df[cols], pd.DataFrame(X, index=meta_df.index)], axis=1
    )
...
def get_user_stats(
    df_with_ids, user_id="usr_id_str", record_id="id_str", agg_type="mean"
):
    user_df = (
        df_with_ids.drop(record_id, axis=1).groupby(user_id).agg(agg_type).reset_index()
    )
    renamed_cols = [
        "%s_usr_%s" % (col, agg_type)
        if (col != user_id) and (col != record_id)
        else col
...
def get_centralities(df, record_id, user_id, centrality_df):
    ...
    df_with_centrality = df[[record_id, user_id]].merge(
        centrality_small, on=user_id, how="left"
    )
...
def load_node_embeddings(file_path, user_id="usr_id_str", users=None):
    ...
    renamed_cols = [
        "%s_ne" % col if col != user_id else col for col in node_emd_df.columns
    ]
    node_emd_df.columns = renamed_cols
    return node_emd_df


def user_label_encoding(
    tr_meta, tr_labels, te_meta, te_labels, record_id="id_str", user_id="usr_id_str"
):
    p1 = tr_meta.copy()
    p1["LABEL"] = tr_labels
    p2 = te_meta.copy()
    ...
    agg_df = calculate_sequential_stats(
        tmp_df,
        record_id,
        user_id,
        value_cols=["LABEL"],
...
def space2json(params: list):
    ...
    for setting in params:
        ...
        out[skey] = {"type": stype, "values": svalues}
    return out
...
def suggest_config(parameters: list, trial):
    ...
    for c_name, c_type, c_vals in parameters:
        if c_type == "choice":
            ...
            config[c_name] = trial.suggest_int(
                c_name,
                c_vals[0],
                c_vals[1],
                step=c_vals[2] if len(c_vals) > 2 else 1,
                log=c_vals[3] if len(c_vals) > 3 else False,
            )
        elif c_type == "float":
            config[c_name] = trial.suggest_float(
                c_name,
                c_vals[0],
                c_vals[1],
                step=c_vals[2] if len(c_vals) > 2 else 1,
